...
public class RateLimiterConfigurationManager {
	...
	// @Autowired
	// private StaticStateManager<StaticStatePersistedOnFileSystemEntity>
	// staticStateManager;
		...
		configurationMap = CacheBuilder.newBuilder()
				.removalListener(new RemovalListener<UUID, RequestUploadProcessingConfiguration>() {
					...
					public void onRemoval(
							RemovalNotification<UUID, RequestUploadProcessingConfiguration> notification) {
						log.debug("removal from requestconfig of " + notification.getKey() + " because of "
				...
				.build(new CacheLoader<UUID, RequestUploadProcessingConfiguration>() {
					...
					@Override
					public RequestUploadProcessingConfiguration load(UUID arg0) throws Exception {
	...
	void remove(RemovalCause cause, UUID key) {
		...
		if (cause.equals(RemovalCause.EXPIRED)) {
			...
			final StaticStatePersistedOnFileSystemEntity entityIfPresentWithIdentifier = staticStateManagerService
					.getEntityIfPresent(key);

			if (entityIfPresentWithIdentifier != null) {

				// if one of the file is not complete, it is not a natural
				// removal
				// but a
				// timeout!! we propagate the event
				for (Entry<UUID, StaticFileState> lavds : entityIfPresentWithIdentifier.getFileStates().entrySet()) {
					if (!lavds.getValue().getStaticFileStateJson().getCrcedBytes()
							.equals(lavds.getValue().getStaticFileStateJson().getOriginalFileSizeInBytes())) {
