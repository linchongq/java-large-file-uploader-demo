...
@Component("uploadServletAsyncHttpRequestHandler")
public class UploadServletAsyncHttpRequestHandler implements HttpRequestHandler {
	private static final Logger log = LoggerFactory.getLogger(UploadServletAsyncHttpRequestHandler.class);
	...
	public void handleRequest(final HttpServletRequest request, final HttpServletResponse response)
		...
		try {
			...
			final FileUploadConfiguration process = fileUploaderHelper.extractFileUploadConfiguration(request, file);
			...
			// get the model
			StaticFileState fileState = staticStateManager.getEntityIfPresent().getFileStates()
					.get(process.getFileId());
			if (fileState == null) {
			...
			// add a listener to clear bucket and close inputstream when process
			// is complete or with error
			...
			// then process
			uploadServletAsyncProcessor.process(fileState, process.getFileId(), process.getCrc(),
					process.getInputStream(), new WriteChunkCompletionListener() {
						...
						public void error(Exception exception) {
							// handles a stream ended unexpectedly , it just
							// means the user has
							// stopped the
							// stream
