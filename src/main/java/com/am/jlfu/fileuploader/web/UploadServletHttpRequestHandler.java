...
@Component("uploadServletHttpRequestHandler")
public class UploadServletHttpRequestHandler implements HttpRequestHandler {
	private static final Logger log = LoggerFactory.getLogger(UploadServletHttpRequestHandler.class);
	...
	public void handleRequest(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			UploadServletAction actionByParameterName = UploadServletAction
					.valueOf(fileUploaderHelper.getParameterValue(request, UploadServletParameter.action));
	...
	private void checkAuthorization(HttpServletRequest request, UploadServletAction actionByParameterName)
		...
		// check authorization
		// if its not get progress (because we do not really care about
		// authorization for get
		// progress and it uses an array of file ids)
		if (!actionByParameterName.equals(UploadServletAction.getProgress)) {

			// extract uuid
			final String fileIdFieldValue = fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId,
					false);
			...
			// call authorizer
			authorizer.getAuthorization(request, actionByParameterName, clientOrJobId,
	...
	private Serializable processAction(UploadServletAction actionByParameterName, HttpServletRequest request)
		...
		case getConfig:
			String parameterValue = fileUploaderHelper.getParameterValue(request, UploadServletParameter.clientId,
					false);
			returnObject = uploadProcessor.getConfig(parameterValue != null ? UUID.fromString(parameterValue) : null);
			break;
		...
		case prepareUpload:
			...
			break;
		case clearFile:
			uploadProcessor.clearFile(
					UUID.fromString(fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId)));
			break;
		case clearAll:
			uploadProcessor.clearAll();
			break;
		case pauseFile:
			List<UUID> uuids = getFileIdsFromString(
					fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId));
			uploadProcessor.pauseFile(uuids);
			break;
		case resumeFile:
			returnObject = uploadProcessor.resumeFile(
					UUID.fromString(fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId)));
			break;
		case setRate:
			uploadProcessor.setUploadRate(
					UUID.fromString(fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId)),
	...
	private Serializable getProgress(HttpServletRequest request) throws MissingParameterException {
		Serializable returnObject;
		String[] ids = new Gson().fromJson(fileUploaderHelper.getParameterValue(request, UploadServletParameter.fileId),
				String[].class);
	...
	private Serializable prepareUpload(HttpServletRequest request) throws MissingParameterException, IOException {

		// extract file information
		PrepareUploadJson[] fromJson = new Gson().fromJson(
				fileUploaderHelper.getParameterValue(request, UploadServletParameter.newFiles),
				PrepareUploadJson[].class);
	...
	private Boolean verifyCrcOfUncheckedPart(HttpServletRequest request)
		...
		try {
			...
			// no need to log this exception, a fallback behaviour is defined in
			// the
			// throwing method.
