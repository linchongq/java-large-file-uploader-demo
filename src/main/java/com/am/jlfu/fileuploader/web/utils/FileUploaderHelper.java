...
public class FileUploaderHelper {
	...
	public FileUploadConfiguration extractFileUploadConfiguration(HttpServletRequest request, MultipartFile file)
		...
		// if (item == null) {
		// throw new
		// JavaFileUploaderException(ExceptionCodeMapping.NoFileToUploadInTheRequest);
		// }
		if (file == null) {
		...
		fileUploadConfiguration.setInputStream(file.getInputStream());
	...
	public void writeExceptionToResponse(final JavaFileUploaderException e, ServletResponse servletResponse)
			throws IOException {
		writeToResponse(
				new SimpleJsonObject(Integer.valueOf(e.getExceptionCodeMapping().getExceptionIdentifier()).toString()),
				servletResponse);
	}

	public void writeToResponse(Serializable jsonObject, ServletResponse servletResponse) throws IOException {
